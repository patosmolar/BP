...
def index():
    ...
    oauth2_client = discovery.build('oauth2','v2', http_auth)
    user_info= oauth2_client.userinfo().get().execute() #THIS FAILS
    givenName = user_info['email']
    givenName = givenName.split('@')
    print("\n"+givenName[0])
    src = "https://calendar.google.com/calendar/embed?src="+givenName[0]+"%40gmail.com&ctz=Europe%2FPrague"
    return flask.render_template('index.html',src=src)
